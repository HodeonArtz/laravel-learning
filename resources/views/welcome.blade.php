          ...
          <div class="colcarousel-inner rounded overflow-hidden  h-100">
            @foreach ($films as $key => $film)
              <div class="carousel-item bg-light h-100 {{$key === 0 ? "active" : ""}}">
                ...
                <img src="{{$film["img_url"]}}" class="d-block h-100 w-100 object-fit-cover" alt="{{$film["name"]}}">
              </div>
            @endforeach
          </div>
    ...
    <section>
      ...
      <form action="/filmin/create" method="POST" class="d-flex flex-column gap-3 col-lg-4">
        ...
        <div>
          ...
          <input class="form-control" required type="text" name="name" id="register-film-name" placeholder="Nombre de la película">
        </div>
        <div>
          <label class="form-label" for="register-film-year">Año</label>
          <input class="form-control" required type="number" min="0"  name="year" id="register-film-year" placeholder="Año de estreno">
        </div>
        <div>
          <label class="form-label" for="register-film-genre">Género</label>
          <input class="form-control" required type="text" name="genre" id="register-film-genre" placeholder="Género de la película">
        </div>
        <div>
          <label class="form-label" for="register-film-country">País</label>
          <input class="form-control" required type="text" name="country" id="register-film-country" placeholder="País donde se ha estrenado">
        </div>
        <div>
          <label class="form-label" for="register-film-duration">Duración (min.)</label>
          <input class="form-control" required type="number" min="0"  name="duration" id="register-film-duration" placeholder="Duración de la película en minutos">
        </div>
        <div>
